...
install-just:
	...
	*) echo "Unsupported OS/arch: $host_os-$arch" >&2; exit 1;; \
	...
	cp "$just_bin" tools/bin/just; chmod +x tools/bin/just; echo "just installed to tools/bin/just"; rm -rf "$tmpdir";
...
install-pnpm:
	...
	*) echo "Unsupported OS/arch or missing mapping: $host_os-$arch" >&2; exit 1;; \
	...
	mkdir -p tools/bin; cp "$pnpm_bin" tools/bin/pnpm; chmod +x tools/bin/pnpm; echo "pnpm installed to tools/bin/pnpm"; rm -rf "$tmpdir";
